...
const Register = () => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    dispatch(registerUser({ email, password, name, surname })).then(
      (res: any) => {
        console.log(res);
    ...
    <div className="authBlock">
      <div className="authBlock__text gap-16">
        ...
        <p className="authBlock__paragraph">
          Заполните простую форму, чтобы создать учетную запись и получить
          доступ ко всем возможностям нашего сайта.
        </p>
      </div>
      <div className={classNames("authBlock__form", c.authBlock__form)}>
        <h2 className="authBlock__h2">Регистрация</h2>
          ...
          <div className="authBlock__form_inputs">
            {FORM_FIELDS.map((field) => (
              <div key={field.id} className="authBlock__form-group">
                {errors[field.id] && (
                  <span style={{ color: "red" }}>{errors[field.id]}</span>
                )}
                <input
            ...
            <div className={c.politics}>
              {errors.checkbox && (
                <span className={c.checkbox_error}>{errors.checkbox}</span>
              )}
              <Checkbox name="checkbox" className={c.checkbox} />
              <span>
                Соглашаюсь на обработку моих персональных данных в соответствии
                с<Link to={POLITICS_URL}>Политикой конфиденциальности</Link>.
              </span>
            </div>
          </div>
          <button
            type="submit"
            className={classNames("action_button", c.button)}
          >
            Создать аккаунт
          </button>
          <Link
