...
export const Filter = ({ filters, setFilters }: FilterProps) => {
  ...
  const chooseFilter = (
    filterId: string,
    element: { key: number | string; value: string }
  ) => {
    setFilters((prev) => {
  ...
  return (
    <div
      className={`${c.photogallery_filter} ${
        dropdownOpen ? "filter filter--open" : "filter"
      }`}
    ...
    >
      ...
      <div className="filter__menu">
        <button
          className={classNames("closeFilter")}
          onClick={() => setDropdownOpen((prevState) => !prevState)}
        >
          &times;
        </button>
          ...
          {FILTERS.map((filter) => (
            <li
              ...
              className={classNames(
                "filter__menu-item",
                activeSelect === filter.id ? c._active : ""
              )}
            >
              <div
              ...
              <div className={c.filter_select}>
                {filter.elements.map((element) => (
                  <div
                    key={element.key}
                    onClick={() => chooseFilter(filter.id, element)}
                  >
                    {element.value}
                  </div>
                ))}
