...
const PhotoGalleryPage = () => {
  ...
  const { filtredPhotoGallery, albums } = useFilterPhotoGallery(
    photoGallery,
    filters,
    album
  );
          ...
          <nav className="tab-nav">
            {albums.length &&
              ["Все альбомы", ...albums].map((item) => (
                <button
                  key={item}
                  className={`button button--white tab-button ${
                    item === album ? "tab-button--active" : ""
                  }`}
                  type="button"
