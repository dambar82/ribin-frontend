          ...
          <div className={styles.quiz}>
            <div className={styles.quiz__content}>
              {currentQuestion === questions.length + 1 ? (
                <div className={styles.quiz__result}>
                  ...
                  <div>
                    <span>{result}</span>/<span>{questions.length}</span>
                  </div>
                </div>
              ) : (
                <>
                  <div className={styles.quiz__header}>
                    <button
                      className={`${styles.quiz__backButton} button`}
                      type="button"
                    >
                      <span>Назад</span>
                    </button>
                  ...
                  <div className={styles.quiz__body}>
                    {currentQuestion === 0 ? (
                      <p className={styles.quiz__invitation}>
                        Готовы проверить свои знания о нашем клубе? Вас ждут
                        увлекательные вопросы о первых шагах, знаменитых
                        победах, великих игроках и незабываемых матчах "Рубина".
                      </p>
                    ) : (
                      <div className={styles.quiz__question}>
                        <p className={styles.quiz__questionText}>
                          {question.text}
                        </p>
                        {answer !== null ? (
                          <div className={styles.quiz__rightAnswer}>
                            <div className={styles.quiz__rightAnswerBody}>
                              {answer ? (
                                <div
                                  className={styles.quiz__rightAnswerResult}
                                  data-result="true"
                                >
                                  Верно
                                </div>
            ...
            <button
              className={
                currentQuestion === 0
                  ? styles.quiz__button
                  : `${styles.quiz__button} ${styles.quiz__button_outlined}`
              }
              onClick={handleClick}
              type="button"
            >
