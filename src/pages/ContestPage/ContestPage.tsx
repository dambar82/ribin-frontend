...
const ContestPage = () => {
  ...
  const { contests, contestStatus, error } = useSelector(
  ...
  const contest: Contest = contests.find(
    (contest) => contest.id.toString() === contestId
  );
    ...
    if (contest) {
      setTopParticipants(
        contest.participants
          .filter((participant) =>
            contest.prizes.some(
              (prize) => prize.client_id === participant.client.id
            )
          )
          .sort((a, b) => {
            const prizeA = contest.prizes.find(
              (prize) => prize.client_id === a.client.id
            );
            const prizeB = contest.prizes.find(
              (prize) => prize.client_id === b.client.id
            );
        ...
        <div className={styles.whitePart}>
          <div className={styles.card}>
            ...
            <div className={styles.card__right}>
              <div
                className={`${styles.card__right_header} justify_content_SB`}
              >
                <h3>{contest?.name}</h3>
                {contest?.status === 1 ? (
              ...
              <div className={styles.card__right_footer}>
                <div className={`${styles.boxShadow} ${styles.datesBlock}`}>
                  <div className={styles.date}>
                    <div className={styles.date_text}>Старт</div>
                    <div className="start_button_minimized">
                      {contest && formatDate(contest.start_date)}
                    </div>
                  </div>
                  <div className={styles.date}>
                    <div className={styles.date_text}>Конец</div>
                ...
                <div className={`${styles.boxShadow} ${styles.date}`}>
                  <div className={styles.date_text}>Объявление результатов</div>
        ...
        {participating === false && contest.status === 1 && (
          <div className={styles.grayPart}>
            <Link to={`/contests/${contestId}/form`}>
              <div className="action_button">Принять участие</div>
            </Link>
          </div>
        )}
      </div>
          ...
          <div className={`section ${styles.results}`}>
            ...
            <div className="section_body">
              {participating && contest.status === 1 && (
                <div className={`shadowBlock ${styles.results_content}`}>
                  <div className={`${styles.results_content_leftPart}`}>
                    <h1>Поздравляем, ты стал (а) участником конкурса!</h1>
                    <div className={styles.timerBlock}>
                      <h2>Осталось времени до оглашения результатов</h2>
                            ...
                            return (
                              <React.Fragment key={timeItem}>
                                <div className={styles.timerItem}>
                                  <div className={styles.timerItem_value}>
                                    {timeLeft[timeItem]}
                                  </div>
                                  <div className={styles.timerItem_label}>
                                    {timeMap[timeItem]}
                                  </div>
                                </div>
              ...
              {contest.status === 0 && topParticipants.length > 0 && (
                <div
                  className={`shadowBlock ${styles.results_content} ${styles.prizes_content}`}
                >
                      ...
                      <div className={styles.prizes_content_block}>
                        <div className={styles.prizes_content_block_left}>
                          ...
                          <div className={styles.prizes_content_block_info}>
                            <div
                              className={styles.prizes_content_block_info_name}
                            >
                              {member.client.name} {member.client.surname}
                            </div>
                            <div
                              className={styles.prizes_content_block_info_level}
                            >
                              <div>
          ...
          <div className={`section ${styles.participants}`}>
            ...
            <div className="section__body">
              <div className={styles.participants_content}>
                {contest &&
                  contest.participants.slice(0, 20).map((participant) => (
